...
```
[Trade Size] [Raceway Type] Nipple, [Length], [Material/Finish]
```
...
| `LENGTH`       | Total length of the nipple; industry shorthands include "Chase", which are the smallest length standard for the size, "Chase", which are nipples with a flat locknut affixed at one end. |
...
- Specify **raceway type** as "Rigid," "IMC," or "Rigid/IMC" if the nipple is dual-rated for both (common, since RMC and IMC share the same OD and thread spec at a given trade size).
- State **material and finish** — galvanized steel, hot-dip galvanized (HDG) steel, or aluminum are the most common; this affects both cost and corrosion resistance.
...
- Many vendor catalogs (e.g., Killark, Conversions Tech, Graybar) also carry a **manufacturer catalog number**; when ordering, provide both the plain-language description above and the catalog number if known, since catalog numbers are not standardized across manufacturers.
